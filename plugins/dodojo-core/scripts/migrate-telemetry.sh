...
LEGACY="${HOME}/.claude"
CANONICAL="${DODOJO_TELEMETRY_HOME:-${LEGACY}/plugins/data/dodojo-core}"
# Phase 1-3 telemetry dir (pre-rename). Migrated forward into CANONICAL if present.
LEGACY_PLUGIN_DATA="${LEGACY}/plugins/data/dodojo-dodojo"
MODE="copy"
DRY=0
for arg in "$@"; do
  case "$arg" in
...
# Step 1 — pre-namespace-migration data (~/.claude/sessions, ~/.claude/hooks)
...
# Step 2 — Phase 1–3 telemetry dir (dodojo-dodojo → dodojo-core)
if [[ -d "$LEGACY_PLUGIN_DATA" && "$LEGACY_PLUGIN_DATA" != "$CANONICAL" ]]; then
  echo "Migrating Phase 1-3 telemetry dir: $LEGACY_PLUGIN_DATA → $CANONICAL"
  migrate_dir "$LEGACY_PLUGIN_DATA/sessions" "$CANONICAL/sessions" "*.jsonl"
  for log in smart-context.log model-route.log skill-suggest.log; do
    migrate_dir "$LEGACY_PLUGIN_DATA/hooks" "$CANONICAL/hooks" "$log"
  done
fi
